...
- **single instruction, multiple data (SIMD)**
  - same operation performed on a number of different data values _in parallel_
- **AVX**
  - advanced vector extensions
  - manage data in sets of registers, **YMM**
    - 256 bits
    - 8 32-bit values, or
    - 4 64-bit values
    - _either_ integer _or_ floating point
  - **scalar** floating-point data
- **AVX2**
  - can be generated by GCC using `-mavx2`
- Media registers: `%ymm0` - `%ymm15`, `%xmm0` - `%xmm15`
  - used to hold floating-point data
  - `%ymm` holds 32 bytes
  - `%xmm` holds low-order 16 bytes
  - when operating on scalar data:
    - they _only_ hold floating-point data,
    - _only_ the low-order 32 bits (`float`) or 64 bits (`double`) are used
- GCC uses scalar movement ops _only_ to transfer data XMM register <-> memory
  - `vmovss M32 X` - single precision memory to XMM register
  - `vmovss X   M32`
  - `vmovsd M64 X` - double precision
  - `vmovsd X   M64`
- For transferring data between XMM registers,
...
- `vunpcklps %xmm0, %xmm0, %xmm0`
  - interleave the values in two XMM registers and store them in a third
  - `[s3, s2, s1, s0]` and `[d3, d2, d1, d0]` -> `[s1, d1, s0, d0]`
- `vcvtps2pd %xmm0, %xmm0`
  - convert two vector elements to double
  - if `%xmm0` is `[x1, x1, x0, x0]` then result is `[dx0, dx0]`
  - where `dx0` is the result of converting `x` to double precision
- On x86-64, the **XMM** registers are used for:
  - passing floating-point arguments to functions
  - returning floating-point values from them
- Up to 8 floating-point args can be passed in XMM registers `%xmm0` - `%xmm7`
  - additional args passed on stack
- Function returning floating-point returns in register `%xmm0`
- _ALL_ XMM registers are **caller-saved**
  - callee may _overwrite any of_ these registers _without_ first saving it
- eg.
  - `double f1(int x, double y, long z);`
    - `x` in `%edi`, `y` in `%xmm0`, `z` in `%rsi`
  - `double f2(double y, int x, long z);`
    - `x` in `%edi`, `y` in `%xmm0`, `z` in `%rsi`
    - same as `f1`
  - `double f3(float x, double *y, long *z);`
    - `x` in `%xmm0`, `y` in `%rdi`, `z` in `%rsi`

### Defining & Using Floating-Point Constants

- _Unlike_ integer arithmetic operations, AVX floating-point operations _CANNOT_ have immediate values as operands
  - compiler must allocate & initialize storage for _any_ constant values
  - the code then reads the value from memory
...
- `.LC2` and `.LC3` are memory locations
...
- `ucomiss S1, S2` for single precision comparison
- `ucomisd S1, S2` for double precision comparison
- `S2` must be in XMM registers
- `S1` can be either in XMM register or in memory
- Three condition codes are set:
  - `ZF` - zero flag
    - set when two operands are equal
  - `CF` - carry flag
    - set when `S2 < S1`
  - `PF` - parity flag
- for floating-point operations, `PF` is set when either operand is `NaN`
- By convention in C, any comparison is considered to fail when one of args is `NaN`
  - `PF` used to detect such condition
  - `x == x` yields `0` when `x` is `NaN`
- **unordered result**
  - when either operand is `NaN`
  - `ZF`, `CF`, `PF` all set to `1`
  - `jp` - jump on parity
-
